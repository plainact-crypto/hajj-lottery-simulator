  ...
  const runCheck = (event: FormEvent) => {
    ...
    const next: Check[] = [
      answerCheck(
        licensedCompany,
        'شركة السياحة مرخصة',
        'هذه نقطة أساسية؛ الضوابط تقصر تنفيذ رحلات العمرة على شركات السياحة المرخصة.',
        'توقف قبل الدفع. الضوابط تحذر من السماسرة والوسطاء والكيانات غير المرخصة.',
      ),
      answerCheck(
        rafiqExplained,
        'تطبيق رفيق والمستندات',
        'الشركة شرحت لك استخدام تطبيق رفيق ورفع عقد الرحلة وإيصال السداد.',
        'اطلب من الشركة شرح تطبيق رفيق ومتطلبات رفع عقد الرحلة وإيصال السداد قبل السفر.',
      ),
    ];

    const duration = Number(durationDays);
    if (!Number.isFinite(duration) || duration <= 0) {
      next.push({ label: 'مدة البرنامج', status: 'warn', detail: 'أدخل عدد أيام البرنامج حتى يمكن فحص المدة.' });
    } else {
      const max = peakException ? 35 : 15;
      next.push(duration <= max
        ? { label: 'مدة البرنامج', status: 'pass', detail: `المدة ${duration} يوم، داخل الحد الأقصى المحدد لهذه الحالة (${max} يوم).` }
        : { label: 'مدة البرنامج', status: 'fail', detail: `المدة ${duration} يوم تتجاوز الحد الأقصى المحدد لهذه الحالة (${max} يوم). راجع الشركة قبل الدفع.` });
    }

    const mecca = Number(meccaDistance);
    if (!Number.isFinite(mecca) || mecca < 0) {
      next.push({ label: 'سكن مكة', status: 'warn', detail: 'أدخل المسافة التقريبية بين الفندق وساحة الحرم بالمتر.' });
    } else if (mecca > 3000) {
      next.push({ label: 'سكن مكة', status: 'fail', detail: 'المسافة المدخلة تتجاوز 3000 متر، وهي أعلى من الحد المذكور في ضوابط الموسم.' });
    } else if (mecca > 1250) {
      next.push(answerCheck(
        meccaShuttle,
        'سكن مكة والنقل الترددي',
        'المسافة أعلى من 1250 متر وأفدت بوجود نقل ترددي؛ تأكد أيضاً أن الفندق على شارع رئيسي وفي منطقة تتوافر بها الخدمات.',
        'عند تجاوز 1250 متر تشترط الضوابط وقوع الفندق على شارع رئيسي وفي منطقة خدمات مع توفير نقل ترددي للحرم.',
      ));
    } else {
      next.push({ label: 'سكن مكة', status: 'pass', detail: 'المسافة المدخلة لا تتجاوز 1250 متر.' });
    }

    const medina = Number(medinaDistance);
    const nights = Number(medinaNights);
    if (!Number.isFinite(medina) || medina < 0) {
      next.push({ label: 'سكن المدينة', status: 'warn', detail: 'أدخل المسافة التقريبية بين الفندق وساحة الحرم النبوي بالمتر.' });
    } else if (medina > 1400) {
      next.push({ label: 'سكن المدينة', status: 'fail', detail: 'المسافة المدخلة تتجاوز 1400 متر، وهي أعلى من الحد المذكور في ضوابط الموسم.' });
    } else if (!Number.isFinite(nights) || nights < 1) {
      next.push({ label: 'سكن المدينة', status: 'fail', detail: 'الضوابط حددت حداً أدنى للإقامة في المدينة المنورة بليلة واحدة.' });
    } else {
      next.push({ label: 'سكن المدينة', status: 'pass', detail: 'المسافة والإقامة المدخلتان لا تتجاوزان/تقلان عن الحدود الأساسية المذكورة.' });
    }

    if (tripMode === 'land') {
      const year = Number(busYear);
      if (!Number.isFinite(year) || year <= 0) {
        next.push({ label: 'الحافلة البرية', status: 'warn', detail: 'أدخل موديل الحافلة؛ ضوابط 1448 رفعت الحد الأدنى إلى موديل 2022.' });
      } else if (year < 2022) {
        next.push({ label: 'الحافلة البرية', status: 'fail', detail: 'موديل الحافلة أقدم من 2022؛ راجع الشركة قبل الحجز.' });
      } else {
        next.push({ label: 'الحافلة البرية', status: 'pass', detail: 'موديل الحافلة المدخل 2022 أو أحدث. تبقى اشتراطات الفحص والصلاحية والتتبع مسؤولية الشركة والجهات المختصة.' });
      }
    }
  ...
  return (
    <div className="page-container content-page">
      <section className="hero">
        <span className="unofficial-badge">أداة مستقلة غير رسمية</span>
        <h1>فاحص برنامج العمرة 1448هـ</h1>
        <p>راجع أهم نقاط برنامج العمرة قبل الحجز وفق الضوابط المصرية المعلنة لموسم 1448هـ.</p>
      </section>

      <div className="inline-notice">
        <strong>مهم</strong>
        <p>هذه أداة مساعدة وليست اعتماداً حكومياً أو بديلاً عن التحقق من بوابة العمرة والجهات الرسمية. لا تدخل بيانات شخصية أو أرقام جوازات.</p>
      </div>
      ...
      {checks && <section className="card" aria-live="polite" style={{ marginTop: '24px', padding: '24px' }}>
        ...
        <div style={{ display: 'grid', gap: '12px', marginTop: '18px' }}>
          {checks.map((item, index) => <div key={`${item.label}-${index}`} className="inline-notice" style={{ margin: 0 }}><strong>{item.status === 'pass' ? '✓ ' : item.status === 'fail' ? '⚠ ' : '؟ '}{item.label}</strong><p>{item.detail}</p></div>)}
        </div>
      </section>}

      <section className="card" style={{ marginTop: '24px', padding: '24px' }}>
        <h2>المصدر الرسمي</h2>
        <p>اعتمدت وزارة السياحة والآثار المصرية ضوابط تنفيذ رحلات العمرة لموسم 1448هـ بتاريخ 15 يوليو 2026.</p>
        <a className="button" href={OFFICIAL_SOURCE} target="_blank" rel="noreferrer" onClick={() => trackFunnelEvent('official_source_clicked')}>راجع خبر الوزارة الرسمي</a>
      </section>
    </div>
  );
}
