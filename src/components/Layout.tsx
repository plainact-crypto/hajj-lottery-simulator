...
const links = [
  ...
  ['/tools', 'الأدوات'],
  ['/data/hajj-tourism-history', 'البيانات'],
  ...
  return (
    <div className="app-shell">
      ...
      <header className="site-header"><div className="nav-inner">
        <Link to="/" className="brand" aria-label="الصفحة الرئيسية"><span className="brand-mark" aria-hidden="true">◈</span><span>محاكي قرعة الحج</span></Link>
        ...
        <nav className={open ? 'nav-links open' : 'nav-links'} aria-label="التنقل الرئيسي">{links.map(([to, label]) => <NavLink key={to} to={to} end={to === '/'}>{label}</NavLink>)}<a href="/trips" style={{display:'inline-flex',alignItems:'center',gap:'6px',fontWeight:700,textDecoration:'none'}} aria-label="رحلات"><span aria-hidden="true">✈️</span><span>رحلات</span></a></nav>
      </div></header>
      <main><Outlet /></main>
